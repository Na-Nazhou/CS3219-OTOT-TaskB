...
describe('Testing the qupte endpoints:', () => {
  ...
  it('should not get a particular quote with invalid id', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property('message', `Cannot find quote with the id ${quoteId}`);
        done();
      });
  });
  ...
  it('should not get a particular quote with non-numeric id', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        expect(res.status).to.equal(400);
        expect(res.body).to.have.property('message', 'Please input a valid numeric value');
        done();
      });
  });
  ...
  it('should not delete a quote with invalid id', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        expect(res.status).to.equal(404);
        expect(res.body).to.have.property('message', `Quote with the id ${quoteId} cannot be found`);
        done();
      });
  });
  ...
  it('should not delete a quote with non-numeric id value', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        expect(res.status).to.equal(400);
        expect(res.body).to.have.property('message', 'Please provide a numeric value');
        done();
      });
  });
});
